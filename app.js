...
// isSafeLearnMoreUrl + isSafeImageSrc (the image-src allowlist) now live in
// lib.mjs as pure, unit-testable predicates — imported at the top of app.js.
...
// Lives in sync.mjs: stores progress + overrides in a single Postgres row
// keyed by sync_key. User configures URL + anon key + sync_key once, then
// can push / pull. Wired here via initSync() + the header ☁️ button.
...
async function init() {
  ...
  // Inject the exam-id list + Stats re-render the cloud-sync module needs.
  initSync({ examIds: EXAM_IDS, renderStats });
  deferIdle(() => {
    installListenButton();
